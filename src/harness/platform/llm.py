...
class LLM:
    ...
    async def __call__(
        ...
        *,
        ...
        готовым), но это АБСОЛЮТНАЯ точка на часах, не накопитель одного лишь
        времени ожидания лимитера (round 4, Item 1 — предыдущая версия этого
        абзаца утверждала обратное, ошибочно; код не менялся, только текст):
        время самого ответа модели (`agent.run()` внутри `_attempt`) тоже
        утекает из тех же 120с, потому что часы между попытками не
        останавливаются — здоровый, но медленный провайдер оставляет
        ...
        и то, чего этот бюджет НЕ считает (бэкофф-паузы `_attempt` между
        HTTP-попытками) — в докстринге `limiter._ACQUIRE_TIMEOUT_S`.
        """
        model: Model | str = (
    ...
    async def _attempt(
        ...
        *,
        ...
        на время `agent.run()`, значит его латентность молча съедает бюджет
        следующего `slot()` (round 4, Item 1 — подробности и обоснование в
        докстринге `limiter._ACQUIRE_TIMEOUT_S`; предыдущая версия этого
        абзаца звучала так, будто модель вообще не в периметре бюджета — это
        было неточно).
        """
