...
def _outcome_line(en: EnrichedHand) -> str | None:
    ...
    **Асимметрия названа вслух, потому что она есть.** При выигрыше печатается
    ...
    и пришпилено тестом 1.8. Владелец выбрал банк, увидев обе величины. Числа при
    ...
    **Непроверенный вход молчит целиком** (`Verdict.not_checked` непуст). Сейчас
    ...
    вскрытие ею. Стек героя на конец такой руки — исход выдуманного вскрытия, а
    не факт раздачи, и «Отдаёте Z ББ» напечатало бы проигрыш, которого могло не
    быть (`test_an_unverified_showdown_leaves_the_outcome_unsaid`). Правило то
    же, каким `worker.pipeline._hand_zone` понижает зону до «предполагая»: вход,
    часть которого проверить было нечем, точным числом не подаётся. Проверка
    стоит по НЕПУСТОМУ списку, а не по имени пометки: новая пометка в
    `not_checked` — тоже причина промолчать, пока не разобрано, что именно она
    ставит под сомнение.
    """
