...
def went_to_showdown(hand: CanonicalHand, label: str) -> bool:
    ...
    (`explanation.hand_replay`, строка вскрытия): им разойтись негде. Третий
    предикат того же вопроса в системе есть и остаётся отдельным —
    `analysis.tournament._showdown` («герой есть в `hand.showdowns`»): он живёт
    в другой спеке, и эта задача его не трогала. Замер расхождения на 2026-09-12: на
    407 раздачах с чекпоинтом признаки совпали на 406, единственное расхождение
    — скриншотная раздача, а рассказ по турниру берёт руки турнира
    (`list_by_tournament`), куда скриншотные не попадают. Число живёт на той
    базе и той дате, не вечно.
