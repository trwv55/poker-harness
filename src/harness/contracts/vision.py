...
* **Ни позиций, ни кнопки** (реестр A3 плюс решение владельца 2026-09-12).
  Позицию не подписывает ни один тип экрана, а кнопка выводится АРИФМЕТИЧЕСКИ:
  малый блайнд сидит следом за ней, и круг мест строится от малого блайнда — на
  экспорте по порядку хода, на живом столе по номерам мест. Фишка дилера была
  третьим кубиком: обе проверенные модели ставили её не тому игроку (Qwen — в 8
  прогонах из 10), а сверять с ней теперь нечего, потому что вторая сторона
  сверки и есть то, что мы считаем сами.
* **Анте ОДНИМ полем** (решение владельца 2026-09-12, отмена реестра B2):
  `ante_pool_shown` — ровно то, что написано («Все анте: N»). Подушевое делит
  код. Двух полей быть не может, потому что на экране одно число: прежняя схема
  просила различить пул и подушевое, и модель честно записывала единственное
  напечатанное значение дважды — это была ошибка схемы, а не чтения.
* **Карты спрашиваются ОДИН раз** (решение владельца 2026-09-12, отмена решения
  о двойном чтении): только `cards_at_seat`. Второе поле просило у модели
  «независимое» наблюдение, которым оно не было: замер, обосновавший его, сделан
  на ВЫРЕЗКЕ колонки лога, а на полном экране модель подставляла туда же, что и
  у места, либо мусор. На тринадцати прогонах одного экрана расхождение этих
  полей ни разу не указало на верное чтение, зато давало вопрос игроку в 12 из 13.
...
class SeenAction(BaseModel):
    ...
    **Строки идут в том порядке, в каком напечатаны**, и на префлопе этот порядок
    ...
    без подписи (реестр, «Проверка схемы»).

    **Напечатанной позиции здесь нет** (решение владельца 2026-09-12). Метку под
    аватаром (`UTG`, `MP+1`, `BTN`) прежняя схема спрашивала ради сверки с
    восстановленным кругом. Но круг и так строится кодом — из блайндов и порядка
    этих самых строк, — то есть метка не добавляла независимого наблюдения, зато
    добавляла поле, которое модель роняет: на замере 2026-09-12 сверка позиций
    валилась в каждом прогоне у обеих проверенных моделей.
    """
