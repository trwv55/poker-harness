...
интерактивный, и получить приоритет самого срочного класса ему безопаснее, чем
уехать в конец очереди по недосмотру (round 5, Item K.10: раньше таблица тут
называлась «исчерпывающей», и это спорило с `.get(..., 100)` двумя сотнями
строк ниже).
"""
...
class JobsQueue:
    ...
    async def await_user(
        ...
        Слияние `payload` читается точечным `select(Job.payload)`, а не
        `session.get(Job, job_id)`. Причина — цена, а не корректность (round 5,
        Item K.4: прежняя редакция утверждала, что `get()` испортил бы
        диагностику `_apply_transition` через identity map; проверка это
        опровергла — `_apply_transition` в ветке провала откатывается
        безусловно, а откат обнуляет identity map и так). Настоящий довод проще:
        нужен один JSONB-столбец, а `get()` гидратировал бы ORM-объект целиком и
        поселил бы его в сессии до конца транзакции — плата без выгоды.
