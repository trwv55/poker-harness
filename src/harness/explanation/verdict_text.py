...
2. **Метка вердикта отдаётся структурно.** Модель возвращает только тексты;
   ...
   то есть метка не следует за тоном текста. Совпадение ТЕКСТА с меткой этим не
   обеспечивается: его держат промпт и проверка направления (текст обязан
   назвать лучшую линию, а на точке «около нуля» — не упрекать; вторая половина
   проверяется здесь же, при разборе ответа).
