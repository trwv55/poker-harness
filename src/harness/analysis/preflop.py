...
# состав позади решаем): 409 решений на обеих фикстурах, из них 71 шов при
# равновесном ожидании 149.5 — отношение 0.475. Оценка множителя по
# правдоподобию: 0.461, профильный интервал 95% [0.352, 0.558] (daily 0.497,
# pko 0.447).
#
...
# собственного множителя игрока по логарифму 0.200 при профильном интервале 95%
# [0.000, 0.575]; независимая оценка по моментам на 18 игроках, у которых восемь и
# больше решений, даёт 0.228 (наблюдаемые отношения там от 0.00 до 1.08). Обе
# оценки сходятся около 0.2, но ВЫБОРКА ЭТУ ВЕЛИЧИНУ НЕ РАЗРЕШАЕТ: гипотеза «все
# игроки одинаковы» (sd = 0) стоит 0.1 логарифмической единицы, то есть не
# отвергается вовсе. Взят верхний конец интервала (0.575), а не оценка: ошибка от
# слишком узкой полосы односторонняя — bracket-тест объявил бы `strict` вывод,
# который на деле держится на догадке, и это ровно та переоценка уверенности,
# которую правило зоны и должно предотвращать.
#
# Отсюда полоса: 0.461 * exp(+-1.96 * sqrt(0.575^2 + 0.117^2)) = [0.146, 1.458],
# наружу — (0.14, 1.46). Межтурнирный разброс (log-разница 0.106, то есть sd 0.053)
# в неё не добавлен отдельно: он на порядок меньше игроцкого и внутри него сидит.
#
...
# с отрывом в 515 логарифмических единиц), а 97.5-й перцентиль ОДНОГО оппонента
# при измеренном разбросе игроков. Обрезать его равновесием значило бы заявить,
# что шире равновесия своего спота не шовит никто, — этого замер не говорит.
...
# дают 46 точек, чей интервал пересекает ноль, и их ширины распадаются на две
# кучи: 42 точки лежат в 0.93...2.47bb, ещё 4 — в 3.68...4.82bb, и между ними
# пусто. Разрыв 1.20bb вчетверо шире любого промежутка внутри нижней кучи
# (крупнейший там — 0.28bb). Порог стоит в этом разрыве, и ЛЮБОЕ значение из
