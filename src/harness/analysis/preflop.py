...
Последнее и проверяет bracket-тест: EV пересчитывается на НЕСКОЛЬКИХ моделях
поля, и зона `strict` ставится, только когда вердикт одинаков на всех и
...
`_CALL_WIDTH_BAND`). Ошибка, которую правило предотвращает,
...
# ВРЕМЕННО: старая полоса, перестраивается следующим коммитом.
_CALL_WIDTH_BAND: tuple[float, float] = (0.5, 2.0)
...
def zone_for(
    ...
    * `best_tight`, `best_interior`, `best_wide` — вердикты на СЕТКЕ ширин
      ...
      (J8o на 8bb — см. `_CALL_WIDTH_BAND`);
