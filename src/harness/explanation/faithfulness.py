...
ACTION_WORDS: dict[str, tuple[str, ...]] = {
    # Корни, а не слова целиком: «скинули карты», «отложили руку», «сдали» —
    # всё это фолд живым языком, и обе модели им пользовались. Требовать нашего
    # словарного слова значило бы штрафовать текст за то, что он написан
    # по-человечески.
    "fold": ("фолд", "пас", "сброс", "скин", "скид", "отлож", "сдать", "сдач"),
...
#
# Ищутся ОБОРОТЫ, а не одиночные слова. Голое «лучше» отвергало ровно те фразы,
# которых мы от модели и добиваемся («ни один вариант не лучше другого»), а
# выжимка той же точки печатает `лучше: около нуля, оба варианта допустимы` —
# то есть мы показывали слово и отказывались от его отражения. То же с «стоило»:
# «расхождение ничего не стоило» — не упрёк, а его отрицание.
NEAR_ZERO_REPROACH: tuple[str, ...] = (
    "лучше было",
    "лучше бы",
    "было лучше",
    "стоило бы",
