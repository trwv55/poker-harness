...
свежезакреплённого соединения — безопасно: если предыдущий держатель жив,
соединение не могло бы быть выдано повторно (пул отдаёт соединение
эксклюзивно), а если локи на нём остались — их владелец по определению уже
не существует.

**Но «безопасно ВСЕГДА» — только при одном условии, которое обязано быть
названо (round 5, Item M).** `pg_advisory_unlock_all()` пространств имён не
различает: он снимает ВСЕ advisory-локи уровня сессии на этом соединении, чьи
бы они ни были — довод «у нас разные namespace» его не касается. Соединение
берётся из движка ПРИЛОЖЕНИЯ, того же, на котором работают репозитории.
Отсюда инвариант системы:

    на движке приложения никто не берёт advisory-лок УРОВНЯ СЕССИИ, кроме
    этого модуля.

Сегодня он выполняется: единственный другой advisory-лок в системе —
`SessionsRepo.active_or_create` (`memory/repos.py`) — транзакционный
(`pg_advisory_xact_lock`), а транзакционные локи `unlock_all` не видит. Смена
там одного вызова на `pg_advisory_lock` молча отменила бы сериализацию сессий
при первом же вызове модели, и ни один комментарий этого бы не заметил —
поэтому инвариант закреплён тестом `test_sessions_lock_is_transaction_scoped`
...
**Бюджет абсолютный — значит, латентность модели и бэкофф-паузы его тоже
тратят (fix round 4, Item 1; уточнено round 5, Item C).** Дедлайн — точка на
часах, отсчитанная от момента, когда `LLM.__call__` прошла свой
внутрипроцессный семафор (не от входа в неё: round 5, Item K.5 — прежний текст
называл вход, и это было неверно), а не накопитель одного только времени
ожидания лимитера. Секунды внутри `agent.run()` (сам вызов модели) утекают из
общего бюджета, потому что часы между попытками не останавливаются. Здоровый,
но медленный провайдер, ответивший за 100с на первой попытке, оставляет второй
...
Ровно то же верно и для пауз экспоненциального бэкоффа `LLM._attempt` между
HTTP-попытками (`2^n + джиттер`, до ~6.3с суммарно за вызов): они идут ПОСЛЕ
выхода из `slot()` — то есть НЕ держат слот одновременности, пока спят, — но
дедлайн они тратят наравне со всем остальным, потому что он абсолютный.
«Снаружи слота» и «снаружи дедлайна» — разные утверждения, и до round 5 этот
абзац утверждал второе, а код делал ни то ни другое: `_sleep` спал ВНУТРИ
`slot()`, держа один из K слотов вхолостую (round 5, Item C починил код, а
затем текст).

Связь с окном `JobsQueue.reap()` (см. `_ACQUIRE_TIMEOUT_S` ниже) — это связь
...
включает ещё ожидание внутрипроцессного семафора до начала отсчёта — и
остаётся ничем не ограниченным до тех пор, пока задача 18 не поставит
запрос-таймаут на сам вызов провайдера.
...
# другим, см. правку выше в модульном докстринге). Дедлайн абсолютный и
# отсчитывается с момента, когда `LLM.__call__` прошла внутрипроцессный
# семафор (round 5, Item K.5: «от входа в `LLM.__call__`» здесь стояло
# ошибочно — вызов, стоящий в очереди за семафором, начинает свои 120с
# позже). Значит и время ответа модели, и бэкофф-паузы между HTTP-попытками
# (до ~6.3с суммарно за вызов) вычитаются из тех же 120с — они не
# складываются поверх, а делят с ними одни часы. Паузы при этом не держат
# слот одновременности: `_sleep` спит снаружи `slot()` (round 5, Item C) —
# но «не держит слот» и «не тратит дедлайн» это разные утверждения, и верно
# только первое. Связь с 600-секундным окном `reap()` — не гарантия для
# ПОЛНОЙ задачи: она верна для той доли задержки, что вносит ожидание
...
# и то, что этот дедлайн не покрывает (очередь за семафором, сам запрос к
# провайдеру) — и остаётся неограниченным, пока задача 18 не поставит
# запрос-таймаут на сам вызов провайдера.
