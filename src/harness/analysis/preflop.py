...
У формы «около нуля» есть граница по ШИРИНЕ, и вот почему она вторая, а не
часть правила знака. Знак решает, есть ли точечная оценка; ширина решает,
осмысленно ли обещание, которое форма даёт игроку («выбор стоит не больше
столько-то»). На интервале шире `_NEAR_ZERO_MAX_WIDTH_BB` обещание неверно: там
не «решение неважное», а отсутствие ответа, и такая точка снова возвращается
...
# полосе оно верно, и замер это подтверждает: если считать полосу отклонением
# КАЖДОГО МЕСТА в отдельности и брать интервал по перцентилям 10-90, а не по
# краям, на референсном споте `TM6292955427` он сужается с 1.153 до 0.312bb на
# прежней полосе и с 1.300 до 0.460bb на нынешней (по 40 точных пересчётов EV
# на каждую строку; числа и способ — в отчёте задачи).
#
...
# ПОЛОСА ДИАПАЗОНА ШОВЕРА — МНОЖИТЕЛИ РАВНОВЕСИЯ, ПО ЗАМЕРУ.
#
# Прежде здесь стояли АБСОЛЮТНЫЕ доли комбо (0.05...1.00), и один и тот же край
# значил в разных спотах разное: равновесный шов на этих же 318 руках занимает от
# 12.5% до 100% комбо (медиана 29.0%), поэтому край 1.00 — это от x1.0 до x8.0
# равновесия, а край 0.05 — от x0.05 до x0.4. Полоса, заданная так, не сужается
# там, где модель уверена, и не расширяется там, где нет.
#
# ЧТО ЗАМЕРЕНО (тот же способ, которым откалибрована `_CALL_WIDTH_BAND`: модель
# даёт вероятность действия, наблюдение даёт 0/1, множитель ширины оценивается по
# правдоподобию). Стол переигрывается по КАЖДОМУ месту — точки решения движок
# снимает только для героя, поэтому героем по очереди становится каждый игрок.
# Форма ровно та, которую модель и считает (банк не открыт, сыгран шов или пас,
# состав позади решаем): 408 решений на обеих фикстурах, из них 71 шов при
# равновесном ожидании 143.2 — отношение 0.496. Оценка множителя по
# правдоподобию: 0.492, профильный интервал 95% [0.383, 0.595] (daily 0.542,
# pko 0.467).
#
# ПОЧЕМУ ПОЛОСА ШИРЕ ЭТОГО ИНТЕРВАЛА. `_CALL_WIDTH_BAND` двигает все места
# позади героя СИНХРОННО и потому описывает стол целиком; здесь же диапазон
# приписывается ОДНОМУ названному оппоненту, и ширина полосы обязана вместить
# разброс игроков между собой, а не только неопределённость среднего. Имена
# оппонентов в фикстурах сквозные (один игрок встречается в десятках рук),
# поэтому разброс измерим: иерархическая подгонка по 69 игрокам даёт sd
# собственного множителя игрока по логарифму 0.082 при профильном интервале 95%
# [0.000, 0.525]; независимая оценка по моментам на 18 игроках, у которых восемь и
# больше решений, даёт 0.121 (наблюдаемые отношения там от 0.00 до 1.08). Обе
# оценки сходятся около 0.1, но ВЫБОРКА ЭТУ ВЕЛИЧИНУ НЕ РАЗРЕШАЕТ: гипотеза «все
# игроки одинаковы» (sd = 0) не стоит ничего (0.0 логарифмической единицы), то
# есть не отвергается вовсе. Взят верхний конец интервала (0.525), а не оценка:
# ошибка от слишком узкой полосы односторонняя — bracket-тест объявил бы `strict`
# вывод, который на деле держится на догадке, и это ровно та переоценка
# уверенности, которую правило зоны и должно предотвращать.
#
# Отсюда полоса: 0.492 * exp(+-1.96 * sqrt(0.525^2 + 0.112^2)) = [0.172, 1.410].
# Межтурнирный разброс (log-разница 0.150, то есть sd 0.075) в неё не добавлен
# отдельно: он в семь раз меньше игроцкого и внутри него сидит.
#
# КОНСТАНТА ШИРЕ ЭТОГО ЗАМЕРА С ОБЕИХ СТОРОН, И ЭТО НЕ ОПИСКА. Полоса замерена
# заново после правки порядка мест на входе решателя (порядок хода вместо порядка
# мест за столом): прежний замер на том же способе давал 0.461 и полосу
# [0.146, 1.458], из которой константа и получена округлением наружу. Новый замер
# уже: [0.172, 1.410]. Константа оставлена прежней намеренно — сужать полосу
# значит объявлять `strict` там, где вывод держится на догадке, а это та самая
# односторонняя ошибка, ради которой берётся верхний конец интервала, а не
# оценка. Число (0.14, 1.46) — решение владельца, и менять его надо вместе с ним.
#
# Верхний конец ВЫШЕ равновесия, в отличие от колл-полосы, и это следствие того же
# различия: 1.46 — не утверждение о поле (как популяционный множитель он отвергнут
# с отрывом в 590.9 логарифмической единицы), а верхний конец полосы вокруг
# ОДНОГО оппонента при измеренном разбросе игроков (сам 97.5-й перцентиль —
# 1.41). Обрезать его равновесием значило бы заявить, что шире равновесия своего
# спота не шовит никто, — этого замер не говорит.
_SHOVE_WIDTH_BAND: tuple[float, float] = (0.14, 1.46)

# Пять точек, равномерных по логарифму множителя, — та же конструкция и та же
# причина, что у `_SHOVE_CALL_WIDTH_MULTIPLIERS`: множитель есть отношение, и
# симметричная по нему мера логарифмическая. Сетка выводится из полосы формулой,
# а не набирается руками
# (`test_the_shover_width_grid_is_built_from_the_measured_band`).
_SHOVER_WIDTH_MULTIPLIERS: tuple[float, ...] = tuple(
    round(_SHOVE_WIDTH_BAND[0] * (_SHOVE_WIDTH_BAND[1] / _SHOVE_WIDTH_BAND[0]) ** (k / 4), 4)
    for k in range(5)
)
...
#
# Саму строку игрок пока не видел ни разу: спота, где вторая ось двигает вердикт,
# а первая при этом даёт точечную оценку, не нашлось ни перебором (тот же тест,
# докстринг), ни на 318 руках фикстур. Ветка оставлена как правило, а не как
# наблюдение: узкий конец полосы диапазона шовера срабатывает раньше почти везде,
# и то, что он срабатывает ВЕЗДЕ, из этого не следует.
...
# ГРАНИЦА, ЗА КОТОРОЙ «ОКОЛО НУЛЯ» ЗНАЧИТ НЕ ТО, ЧТО ГОВОРИТ.
#
# Форма выше обещает игроку освободить внимание: оба варианта допустимы, дороже
# названного потолка выбор не стоит. Обещание держится, только пока интервал
# узкий. На широком интервале точечная оценка около нуля не значит ничего: знак
# не установлен, порядок величины не установлен, а потолок вместо «здесь нельзя
# много потерять» говорит «мы не знаем, сколько». Это не «решение неважное», это
# отсутствие ответа, и называть их одной формой нельзя.
#
# ГДЕ ПРОХОДИТ ГРАНИЦА — ПО ЗАМЕРУ РАСПРЕДЕЛЕНИЯ ШИРИН, А НЕ ПО ВКУСУ. На
# калиброванных полосах (`_CALL_WIDTH_BAND`, `_SHOVE_WIDTH_BAND`) обе фикстуры
# дают 48 точек, чей интервал пересекает ноль, и их ширины распадаются на две
# кучи: 44 точки лежат в 0.93...2.47bb, ещё 4 — в 3.68...4.82bb, и между ними
# пусто. Разрыв 1.20bb вшестеро шире любого промежутка внутри нижней кучи
# (крупнейший там — 0.19bb). Порог стоит в этом разрыве, и ЛЮБОЕ значение из
# 2.47...3.68 разбивает выборку одинаково — число внутри разрыва не выбирается,
# а разрыв замерен. Закреплено
# `test_the_near_zero_form_is_refused_to_an_interval_that_is_too_wide`.
_NEAR_ZERO_MAX_WIDTH_BB = 3.0
...
def _too_wide_for_near_zero(interval: EvInterval) -> str:
    """Причина снять вердикт с точки, чей интервал через ноль шире `_NEAR_ZERO_MAX_WIDTH_BB`.
    ...
    Причина написана для игрока (SESSIONS_UX): она называет, на сколько
    расходятся модели и почему числа не будет, без внутренних терминов.
    """
    width = interval.high_bb - interval.low_bb
...
def _shover_range_models(
    solution: MultiwaySolution, multipliers: Sequence[float] = _SHOVER_WIDTH_MULTIPLIERS
) -> dict[str, Range]:
    """Семейство диапазонов шовера: равновесие его подыгры, умноженное на полосу.

    Ключ — множитель (`_WIDTH_KEY`), значение — диапазон ширины
    `множитель * равновесная ширина`, набранный по порядку классов САМОГО
    равновесия: сначала по весу шова, при равном весе — по эквити против
    диапазонов тех, кто на этот шов отвечал. Порядок берётся из решения, а не
    задаётся снаружи, ровно затем, чтобы на множителе x1 семейство
    воспроизводило модель, а не подменяло её другим диапазоном той же ширины.

    Замер по 29 точкам «колл шова» обеих фикстур — доля веса равновесного шова,
    попавшая в построенный на x1 диапазон: медиана 0.995, минимум 0.976. Для
    сравнения, ранжирование по эквити против равновесного шова той же глубины
    (то, что делал прежний `range_of_width`) даёт 0.906 и 0.793, а по эквити
    против коллеров — 0.966 и 0.913. Нижняя граница закреплена
    `test_the_shover_width_family_reproduces_the_equilibrium_at_one`.
    """
    by_equity = classes_by_equity_against(_average_range(list(solution.calls)))
    rank = {cls: index for index, cls in enumerate(by_equity)}
    ordered = sorted(rank, key=lambda cls: (-solution.push.weight(cls), rank[cls]))
    width = solution.push.fraction_of_hands()
    return {
        _WIDTH_KEY(multiplier): range_of_width_in_order(ordered, min(1.0, width * multiplier))
...
def _unopened_verdict(dp: DecisionPoint, en: EnrichedHand, state: TableState) -> PointVerdict:
    ...
    # Форма «около нуля» обещает, что выбор дёшев; на слишком широком интервале
...
def _facing_shove_verdict(dp: DecisionPoint, en: EnrichedHand, state: TableState) -> PointVerdict:
    ...
    detail: dict[str, object] = {
        ...
        "ev_call_by_width_bb": {key: round(v, 4) for key, v in ev_by_width.items()},
        "hero_class": hero_cls,
...
def verdict_for(dp: DecisionPoint, en: EnrichedHand) -> PointVerdict:
    ...
    **Точка без сыгранного действия — тоже отказ, и названный.** Скриншот живого
    стола застаёт героя ДО хода (`harness.engine.state`): вердикт этого пакета
    сравнивает сыгранное с лучшим, а сыгранного там ещё нет. Ответить «ошибок
