...
async def handle_document(deps: BotDeps, tg_user_id: int, file_bytes: bytes, filename: str) -> Msg:
    ...
    **Повторная загрузка того же файла не считается второй раз** (fix round 1).
    ...
    модульный докстринг), так что обе успевают увидеть «сканов не было». Цена
    ограничена одним лишним сообщением: турнир у обеих задач общий, а
    счётчик-чекпоинт `_run_hh_scan` не даст сохранить руки дважды. Лечить это
    переносом дедупликации на `tournaments` нельзя — тогда сломается повторная
    попытка после смерти процесса между коммитом и `enqueue`, которую тот же
    модульный докстринг бережёт сознательно.
    """
