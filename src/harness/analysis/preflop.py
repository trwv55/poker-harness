...
#
# ШИРИНЫ ЗАДАЮТСЯ МНОЖИТЕЛЯМИ К РАВНОВЕСНОЙ, А НЕ ДОЛЯМИ КОМБО. Вилка обязана
# накрывать вердикт самой модели: иначе `zone_for` честно скажет «ни одна ширина
# вилки его не подтверждает», и точка получит `assuming` — а при развороте внутри
# интервала останется и вовсе без цены — при полностью исправной модели.
# Равновесный колл на многолюдном столе порядка десятой доли комбо, то есть УЖЕ
# самого узкого конца прежней сетки (0.20): ни одна её точка модель не накрывала.
#
