...
тем, что HH-путь модель не зовёт вовсе; первый же вызов из станции упёрся бы в
...
class Trace:
    ...
    def record(self, name: str, **fields: Any) -> None:
        """Записать УЖЕ СЛУЧИВШИЙСЯ шаг — тот, длительность которого мерил не трейс.

        `span` оборачивает работу и засекает время сам; ступени каскада зрения
        (`harness.parsers.vision_adapter`) случаются внутри одного вызова
        адаптера, и обернуть каждую снаружи нечем — конвейерные пакеты про трейс
        не знают (правило зависимостей CLAUDE.md). Адаптер возвращает список
        ступеней значением, а станция переносит их сюда: без этого в трейсе
        осталось бы «зрение отработало», а на каком переходе появилось
        расхождение — нет.
        """
