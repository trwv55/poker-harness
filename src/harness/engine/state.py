...
действий, а последовательности на экране не напечатано. Валидатор при этом не
вправе считать такой вход битым — иначе главный сценарий продукта («кинул скрин
за столом») отвечал бы отказом.
...
на префлопе это в точности стартовый стек (проверено на экранах C и D сверкой с
текстом рума до фишки). На постфлопе вклады прошлых улиц в него не входят — см.
последний пункт выше и `STATE_NOT_CHECKED`.
"""
