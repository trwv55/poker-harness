...
verdict_label_for`), модель её не выбирает и не видит своей задачей: текст
только излагает. Поэтому расхождение «в тексте похвала, в метке mistake»
невозможно по построению, а не по договорённости — метка и число приходят из
одного места.
"""
