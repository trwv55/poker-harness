...
* *Скачок bb на границе уровня объясняется, а не прячется.* Стек на входе
  ...
  выросли. Строка выглядит как сбой данных, поэтому такие переходы
...
# Сколько строк каждого списка уходит в промпт. Списки отсортированы ядром по
# цене, поэтому обрезка сверху оставляет самое дорогое; смысл потолка — не
# отправлять в промпт десятки строк «где ушли фишки» с турнира на 150 раздач.
_MAX_ALL_INS = 5
_MAX_CHIP_MOVES = 5
_MAX_FINDINGS = 4
_MAX_LEVELS = 12
...
def _trajectory_lines(report: TournamentReport, book: NumberBook) -> list[str]:
    ...
    if jumps:
        lines.append(
            ...
            + ", ".join(jumps)
            + ". Это не сбой данных: фишки те же, выросли блайнды."
        )
    lines.append(
