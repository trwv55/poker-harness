...
совпадает с вердиктом самой модели. Опрашивать два конца было бы неверно: EV по
ширине не монотонна, и вердикт умеет перевернуться внутри интервала (см.
`_SHOVE_CALL_WIDTH_MULTIPLIERS`). Ошибка, которую правило предотвращает,
...
**Две проверки, снимающие вердикт с уже посчитанной точки.** Обе о том, чему
верить нельзя, а не о том, что посчитано неверно. Первая — контрольная сумма
модели стола (`_model_checksum`): колл-диапазоны игроков позади берутся из
хедз-ап равновесия и раздаются каждому независимо, и когда по ним выходит, что
шов почти наверняка ответят, модель описывает не этот стол — цену шова по ней
не называем. Вторая — неустойчивая вилка (`_UNSTABLE_SHOVE`, `_UNSTABLE_CALL`):
вердикт, который переворачивается ВНУТРИ интервала правдоподобных ширин,
точечной оценки не получает; исключение — форма, где ширину не угадывают, а
берут из равновесия. Порядок проверок — сначала модель, потом устойчивость.
Посчитанные числа остаются в `detail` снятой точки, игроку не показывается
ничего: `best_action` пуст, и `error_cost.is_judged` такую точку не берёт.
...
# Шов J8o на 8bb с анте стола 1.0bb и двумя игроками позади даёт +0.87 / +0.31 /
# -0.07 / -0.20 / +0.06 bb на множителях 0.5 / 0.75 / 1 / 1.5 / 2: опрос концов
# объявил бы вердикт устойчивым на интервале, внутри которого он дважды меняет
# знак. Поэтому опрашивается сетка, и `strict` требует одного ответа на ВСЕХ её
# точках. Закреплено `test_interior_reversal_of_the_bracket_gets_no_point_estimate`.
#
# ШИРИНЫ ЗАДАЮТСЯ МНОЖИТЕЛЯМИ К РАВНОВЕСНОЙ, А НЕ ДОЛЯМИ КОМБО. Прежняя сетка
# 0.20-1.00 была абсолютными долями комбо, а равновесный колл на многолюдном столе
# — порядка 0.10, то есть уже самого узкого её конца. Она спрашивала «что если
# поле коллирует вдвое-вдесятеро шире решения, против которого судится точка», и
# на половине точек вердикт такого разброса не переживал. Полоса вокруг равновесия
# спрашивает ту величину, которая тут и неизвестна, — насколько поле отклоняется
# от равновесия. Замер обеих фикстур: точек с вердиктом 12 против 8, снятых
# неустойчивой вилкой 30 против 34.
#
# Что теперь означает вилка. Ширина колла КАЖДОГО МЕСТА умножается на множитель —
# места отличаются постами, и равновесные ширины у них разные, — а классы
# упорядочиваются по эквити против равновесного диапазона шова ГЕРОЯ
# (`classes_by_equity_against`), то есть против того шова, на который коллер и
# отвечает. Множители задают, насколько мы допускаем отклонение поля от
# равновесия: от вдвое более тесного поля до вдвое более широкого.
#
# Премиум-конец (`BRACKET_TIGHT`, 3.02% комбо) из этой вилки убран. Он был
# осмыслен, пока колл-диапазон был догадкой: заведомо слишком узкий конец
# ограничивал догадку снизу. Против равновесия он означал бы отдельное
# утверждение — «поле способно коллировать втрое теснее половины равновесия», —
# которого мы не измеряли. В вилке колла против шова (`_SHOVER_RANGE_WIDTHS`)
# узкий конец остаётся: там он описывает нита-шовера, а не поле.
_SHOVE_CALL_WIDTH_MULTIPLIERS: tuple[float, ...] = (0.5, 0.75, 1.0, 1.5, 2.0)

# Ключ ширины в разбивке EV по вилке (`detail`): множитель к равновесной ширине.
...
# Причина снять вердикт при неустойчивой вилке (решение владельца 2026-09-06).
# Точка, у которой вердикт переворачивается ВНУТРИ интервала правдоподобных
# ширин, точечной оценки не получает: показанное число было бы оценкой ровно
...
_UNSTABLE_CALL = (
    "цена этого решения переворачивается в зависимости от того, с какими руками "
    "оппонент идёт олл-ин: одного надёжного числа здесь нет"
)
...
def zone_for(
    ...
    * `best_tight`, `best_interior`, `best_wide` — вердикты на СЕТКЕ ширин
      ...
      (J8o на 8bb — см. `_SHOVE_CALL_WIDTH_MULTIPLIERS`);
...
def _unopened_verdict(dp: DecisionPoint, en: EnrichedHand, state: TableState) -> PointVerdict:
    ...
    # не монотонна (см. `_SHOVE_CALL_WIDTH_MULTIPLIERS`). Ширины отсчитываются от
    # равновесной ширины каждого места, а порядок классов — по эквити против
    # равновесного шова героя: коллер отвечает именно на этот шов.
    ...
    # Неустойчивая вилка точечной оценки не даёт (`_UNSTABLE_SHOVE`). Форма, где
    # колл-диапазон взят из равновесия, а не угадан, — исключение: там вердикт
    # держит равновесие, а не вилка, и `zone_for` ставит такой точке `strict`.
    if bracket_shove == "unstable" and equilibrium_depth is None:
        return _unjudged(dp, spot, _UNSTABLE_SHOVE, detail)
    return PointVerdict(
        dp_index=dp.index,
        street=dp.street,
        spot=spot,
...
def _facing_shove_verdict(dp: DecisionPoint, en: EnrichedHand, state: TableState) -> PointVerdict:
    ...
    # То же правило, что и для шова в неоткрытый банк: при неустойчивой вилке
    # точечной оценки нет, кроме формы, где диапазон шовера взят из равновесия.
    if bracket_call == "unstable" and equilibrium_depth is None:
        return _unjudged(dp, spot, _UNSTABLE_CALL, detail)
    return PointVerdict(
        dp_index=dp.index,
        street=dp.street,
        spot=spot,
...
def cheap_fold_verdict(dp: DecisionPoint, en: EnrichedHand) -> PointVerdict | None:
    ...
    дорогого перебора. Устойчивость вердикта к ширине колл-диапазона он не
    проверяет вовсе, а после `_UNSTABLE_SHOVE` именно она решает, будет ли у
    точки цена: поэтому мультивей уходит на полный расчёт, а лукап закрывает
    только форму, где колл-диапазон взят из равновесия (SB против BB один на
    один) — там вилка вердикт и не держит. Отсюда зона `strict` без допущения:
    вывода на догадке эта функция больше не производит.
    """
    ...
    # Устойчивость вердикта к ширине колл-диапазона лукап не проверяет вовсе, а
    # после правила `_UNSTABLE_SHOVE` именно она решает, будет ли у точки цена.
    # Заявить её здесь нечем, поэтому мультивей уходит на полный расчёт: он
    # вилку считает. Форма с равновесным колл-диапазоном остаётся за лукапом —
    # там вилка вердикт и не держит.
