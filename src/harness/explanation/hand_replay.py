...
улицы, стеки, точки решения героя, исход раздачи. Модуль только форматирует.
...
def _dead_before_deal(hand: CanonicalHand) -> int:
    ...
    не публикует (`pot_by_street` — итог улицы). Второе число блока не от движка —
...
def _outcome_line(en: EnrichedHand) -> str | None:
    ...
    **Асимметрия названа вслух, потому что она есть.** При выигрыше печатается
    банк ЦЕЛИКОМ — вместе с фишками, которые герой положил в него сам; при
    проигрыше — чистая убыль стека. На руке владельца это 2.9 ББ против 2.3 ББ
    чистого прироста, и владелец выбрал банк, увидев обе величины. Числа при
