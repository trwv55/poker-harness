...
**Cairo ищется явно (macOS).** `cairocffi` находит библиотеку через
`ctypes.util.find_library`, а тот на macOS смотрит только в путях dyld по
умолчанию — Homebrew (`/opt/homebrew/lib`, `/usr/local/lib`) в них не входит, и
на машине разработчика импорт падал бы при полностью установленной cairo.
`_locate_cairo` дописывает эти каталоги в `DYLD_FALLBACK_LIBRARY_PATH` процесса
ДО импорта: `find_library` читает переменную в момент вызова, поэтому правка
работает, хотя dyld свою копию окружения уже прочитал. На Linux (образ Docker,
`libcairo2`) ветка не срабатывает вовсе — `find_library` находит библиотеку сам.
"""
...
# Геометрия в пикселях SVG. Клетка 56 px — подпись `AKs` читается на телефоне,
# а вся сетка (13 × 56 + поля) остаётся меньше 800 px по стороне: Телеграм
# показывает такую картинку без открытия в полный экран.
_CELL = 56
_PAD = 12
_TITLE_H = 44
...
def _locate_cairo() -> None:
    """Дать `find_library` шанс найти cairo из Homebrew (см. модульный докстринг)."""
