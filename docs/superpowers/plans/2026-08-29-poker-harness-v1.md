...
- Produces:
  ...
  - `BRACKET_TIGHT`, `BRACKET_WIDE: Callable[[float], Range]` — узкая («только премиум»: AA-JJ, AK) и широкая (top-40%) модели колл-диапазона по глубине; вход bracket-теста зоны (задача 12). **Состав широкой модели выводится из данных, а не подбирается на глаз:** классы упорядочиваются по эквити против случайной руки (эта величина уже есть в предвычисленной таблице) и берётся верхняя доля. Иначе рукотворный список становится единственным невыведенным числом в цепочке, от которой зависит зона доверия вердикта;
...
- [ ] **Step 2: падают.** Step 3: реализация:
  ...
  - `nash_hu(eff_bb)`: fictitious play на предвычисленной таблице эквити.
